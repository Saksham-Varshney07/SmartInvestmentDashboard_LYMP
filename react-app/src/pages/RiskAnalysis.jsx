  ...
  const PortfolioCard = ({ portfolio }) => {
    return (
      <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200 hover:border-slate-300 transition-colors">
        <h3 className="font-bold text-lg text-slate-900 mb-1">{portfolio.name}</h3>
    ...
    <div className="min-h-screen bg-[#f8f9fa] text-slate-800 antialiased font-['Inter']">
      ...
      <main className="max-w-[1240px] mx-auto px-6 py-24 pb-32">
        <div className="flex items-center gap-3 mb-8">
           <span className="material-symbols-outlined text-4xl text-blue-600">health_and_safety</span>
           <h1 className="text-4xl font-bold text-slate-800">AI Portfolio Doctor</h1>
        </div>

        {step === 1 && (
          <div className="max-w-2xl mx-auto bg-white p-10 rounded-2xl shadow-sm border border-slate-200 animate-fade-in text-center mt-12">
            <span className="material-symbols-outlined text-[80px] text-blue-100 mb-6">psychology</span>
            <h2 className="text-2xl font-bold mb-4">Discover Your Investor Profile</h2>
            ...
            <div className="space-y-4 text-left">
              <label className="flex items-start gap-4 p-5 rounded-xl border border-slate-200 hover:border-blue-400 hover:bg-blue-50 cursor-pointer transition-colors relative">
                <input type="radio" name="risk" className="mt-1 w-5 h-5 text-blue-600 accent-blue-600" onChange={() => setRiskProfile('The Defender')} />
                <div>
                  <h4 className="font-bold text-slate-900 mb-1">The Defender (Low Risk)</h4>
                  <p className="text-sm text-slate-500">I prefer capital preservation. A sudden 15% market crash would cause me severe anxiety. I want steady, reliable compounding.</p>
                </div>
              </label>
              ...
              <label className="flex items-start gap-4 p-5 rounded-xl border border-slate-200 hover:border-blue-400 hover:bg-blue-50 cursor-pointer transition-colors relative">
                ...
                <div>
                  <h4 className="font-bold text-slate-900 mb-1">The Balanced Optimist (Medium Risk)</h4>
                  <p className="text-sm text-slate-500">I can stomach temporary 10-20% dips if it means outperforming inflation and bonds. I want a mix of offense and defense.</p>
                </div>
              </label>
              ...
              <label className="flex items-start gap-4 p-5 rounded-xl border border-slate-200 hover:border-blue-400 hover:bg-blue-50 cursor-pointer transition-colors relative">
                ...
                <div>
                  <h4 className="font-bold text-slate-900 mb-1">The Aggressor (High Risk)</h4>
                  <p className="text-sm text-slate-500">I am here for high alpha. I am comfortable with extreme volatility and 40% drops because my timeline is long enough to recover.</p>
                </div>
              </label>
            </div>
        ...
        {step === 2 && (
          <div className="max-w-2xl mx-auto bg-white p-10 rounded-2xl shadow-sm border border-slate-200 animate-fade-in mt-12">
            <button onClick={() => setStep(1)} className="text-sm text-slate-500 flex items-center mb-6 hover:text-slate-800 transition-colors"><span className="material-symbols-outlined text-[16px] mr-1">arrow_back</span> Choose Profile</button>
            <h2 className="text-2xl font-bold mb-8">Construct Your Test Portfolio</h2>
            
            <div className="space-y-8">
              <div>
                <label className="block text-sm font-semibold text-slate-700 mb-2">Total Capital / Budget (₹)</label>
                <div className="relative">
                  <span className="absolute left-4 top-1/2 -translate-y-1/2 font-bold text-slate-400">₹</span>
                  <input type="number" value={budget} onChange={e => setBudget(Number(e.target.value))} className="w-full bg-slate-50 border border-slate-200 rounded-lg py-3 pl-10 pr-4 font-bold text-slate-800 focus:ring-2 focus:ring-blue-500 outline-none transition-all" />
                </div>
              </div>

              <div>
                <label className="block text-sm font-semibold text-slate-700 mb-2">Investment Horizon (Years)</label>
                <input type="number" value={duration} onChange={e => setDuration(Number(e.target.value))} className="w-full bg-slate-50 border border-slate-200 rounded-lg py-3 px-4 font-bold text-slate-800 focus:ring-2 focus:ring-blue-500 outline-none transition-all" />
              </div>

              <div>
                <label className="block text-sm font-semibold text-slate-700 mb-2">Build Your Stock Universe</label>
                <p className="text-xs text-slate-500 mb-4">Add the stocks you ideally want in your portfolio. Our ML engine will cross-examine them for overlapping correlation risk.</p>
                ...
                <div className="flex gap-2 relative">
                  <input 
                    ...
                    className="flex-1 bg-slate-50 border border-slate-200 rounded-lg py-3 px-4 text-slate-800 outline-none focus:ring-2 focus:ring-blue-500 transition-all font-bold" 
                  />
                  <button onClick={addStock} className="bg-slate-800 text-white px-6 font-bold rounded-lg hover:bg-slate-900 transition-colors">Add</button>
                  
                  {showSuggestions && suggestions.length > 0 && (
                    <div className="absolute top-14 left-0 right-24 bg-white border border-slate-200 shadow-xl rounded-xl overflow-hidden z-20">
                      {suggestions.map((s, i) => (
                        <div 
                          key={i} 
                        ...
                        >
                          ...
                          <div className="text-sm text-slate-500 truncate max-w-[200px]">{s.shortname}</div>
                        </div>
                      ))}
                    </div>
                  )}
        ...
        {step === 3 && analysisResult && (
          <div className="animate-fade-in">
             ...
             <h2 className="text-2xl font-bold mb-6 text-slate-900">AI Suggested Allocations</h2>
