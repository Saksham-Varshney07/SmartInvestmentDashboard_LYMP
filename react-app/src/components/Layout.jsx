...
export default function Layout({ children }) {
  ...
  const getNavClass = (path) => {
    return location.pathname === path || (path !== '/' && location.pathname.startsWith(path))
      ? "text-blue-600 font-semibold border-b-2 border-blue-600 pb-1 text-sm tracking-tight"
      : "text-slate-500 hover:text-slate-900 transition-colors text-sm font-medium tracking-tight";
  };

  if (!currentUser) {
    return (
      <div className="min-h-screen bg-slate-50 flex flex-col justify-center items-center p-6 antialiased font-['Inter'] py-12">
        <div className="w-full max-w-md bg-white rounded-2xl shadow-xl p-8 border border-slate-100">
           ...
           <h2 className="text-2xl font-bold text-center text-slate-900 mb-2">{isSignup ? 'Create Account' : 'Welcome Back'}</h2>
           <p className="text-center text-slate-500 text-sm mb-8">{isSignup ? 'Set up your investor profile' : 'Login to view your portfolio.'}</p>
             ...
             {isSignup && (
               <>
                 ...
                 <div className="grid grid-cols-2 gap-4">
                   ...
                   <div>
                     <label className="block text-sm font-medium text-slate-700 mb-1">Horizon</label>
                     <select 
           ...
           <div className="mt-6 text-center text-sm text-slate-500">
             ...
             <button type="button" onClick={() => setIsSignup(!isSignup)} className="text-blue-600 font-bold hover:underline">
  ...
  return (
    <div className="bg-[#f8f9fa] min-h-screen text-slate-800 antialiased font-['Inter'] flex flex-col">
      {/* Global Navbar */}
        ...
        <div className="flex justify-between items-center max-w-[1440px] mx-auto px-6 h-16">
          ...
          <div className="flex items-center gap-4 relative" ref={searchRef}>
            <div className="relative hidden sm:block">
              <input 
                className="bg-slate-100 border-none rounded-lg py-1.5 pl-10 pr-4 text-sm focus:ring-2 focus:ring-blue-500 w-64 outline-none" 
            ...
            <div className="flex items-center gap-3 pl-4 border-l border-slate-200">
               <div className="text-sm font-semibold text-slate-700 hidden lg:block">{currentUser.username}</div>
